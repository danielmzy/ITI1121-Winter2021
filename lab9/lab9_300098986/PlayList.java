...
public class PlayList {
    ...
    public boolean addSong( Song song ) {
        ...
        if ( count == songs.length ) {
            ...
            songs = new Song[ temporary.length + capacityIncrement ];
    ...
    public void sort2( java.util.Comparator<Song> c ) {
        ...
        for ( int i = 0; i < count; i++ ) {
            ...
            for ( int j = i+1; j < count; j++ )
                if ( c.compare( songs[ j ], songs[ minimum ] ) < 0 )
                    minimum = j;
      
            // Swap the smallest unsorted element with the element
            // found a position i.
      
            Song temporary = songs[ minimum ];
            songs[ minimum ] = songs[ i ];
            songs[ i ] = temporary;
        }
    }
