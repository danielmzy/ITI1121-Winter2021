...
public class GainInfoItem {
	...
	public static void reverseSort(GainInfoItem[] items) {
		try {
			GainInfoItem momentary;
			for (int x = 0; x < items.length; x++) {
				for (int y = 1; y < items.length - x; y++) {
					if (items[y].getGainValue() > items[y - 1].getGainValue()) {
						momentary = items[y - 1];
						items[y - 1] = items[y];
						items[y] = momentary;
					}
			...
			System.out.println(" A problem has been found for the reverseSort method in GainInfoItem.java");
		}
	}
}