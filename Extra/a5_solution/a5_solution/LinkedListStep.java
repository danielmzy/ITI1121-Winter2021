...
public class LinkedListStep<E> {
	...
	private int start, stop, step, index;
    
        private LinkedListIterator(int start, int stop, int step) {
	    ...
	    if (step <= 0) {
		throw new IllegalArgumentException("step should be greather than 0 "+step);
	    }
	...
	public E next() {
        ...
        public boolean hasNext() { // cannot be empty by construction
            return index == -1 || index+step <= stop;
        }
    ...
    public Iterator<E> iterator() {
        return new LinkedListIterator(0, size-1, 1);
    }
    ...
    public Iterator<E> iterator(int stop) {
        return new LinkedListIterator(0, stop, 1);
    }
    ...
    public Iterator<E> iterator(int start, int stop) {
        return new LinkedListIterator(start, stop, 1);
    }

    /**
     * Returns an iterator for this list that starts at a specified
     * position and stops at a specified position. The parameter step
     * controls the difference between the visited indices.
     *
     * @param start the index of the first element of the iteration
     * @param stop the index of the last element of the iteration
     * @param step difference between each index in the iteration
     * @return an iterator for this list
     */

    public Iterator<E> iterator(int start, int stop, int step) {
        return new LinkedListIterator(start, stop, step);
    }
